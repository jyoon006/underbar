(function() {
  ...
  _.extend = function(obj) {
	var args = Array.prototype.slice.call(arguments, 1);
	 _.each(args, function(values) {
	  _.each(values, function(val, prop) {
		 obj[prop] = val;
		});
	 });
	 return obj;
  ...
  _.zip = function() {
	...
	for(var i = 0; i < longestLength;	i++) {
	 for(var j = 0; j < args.length; j++) {
	  zippedArr.push(args[j][i]);
	 }
	 result.push(zippedArr);
	 zippedArr = [];
	}
	return result;
  };
  ...
  _.intersection = function() {
	...
	_.each(firstArg, function(value) {
	 _.each(args, function(arr) {
	  if(_.contains(arr, value)) {
		 intersectedArr.push(value);
		}
	 });
	});
